...
const accessTokenPorRefreshToken = async () => {
   try {
    ...
    console.error("Error al pedir access_token:", error.response?.data || error.message);
    throw error;
  }
...
const obtenerAccountId = async (accessToken) => {
  try {
    ...
    console.error('Error al obtener accountId:', error.response?.data || error.message);
    throw new Error('Falló la obtención del accountId 💥');
  }
};

module.exports = { codePorTokensZoho , obtenerAccountId, accessTokenPorRefreshToken};